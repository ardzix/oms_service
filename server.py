...
from checkout.grpc import checkout_pb2_grpc
from checkout.grpc.grpc_services import CheckoutService
...
def serve():
    ...
    checkout_pb2_grpc.add_CheckoutServiceServicer_to_server(CheckoutService(), server)
    ...
    server.add_insecure_port(f"[::]:{settings.OMS_CHECKOUT_SERVICE_PORT}")

    server.start()
    logger.info(
        f"gRPC servers running on ports {settings.OMS_CART_SERVICE_PORT}, {settings.OMS_CHANNEL_SERVICE_PORT} and {settings.OMS_CHECKOUT_SERVICE_PORT}..."
    )
    try:
        while True:
            time.sleep(86400)
